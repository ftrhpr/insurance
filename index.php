...
<body class="bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 text-slate-800 font-sans min-h-screen selection:bg-primary-200 selection:text-primary-900">
    ...
    <!-- Edit Case Modal -->
    <div id="edit-modal" class="hidden fixed inset-0 z-[9999]" role="dialog" aria-modal="true">
        <!-- Backdrop -->
        <div class="fixed inset-0 bg-gradient-to-br from-slate-900/60 via-purple-900/40 to-pink-900/50 backdrop-blur-lg transition-all duration-300" onclick="window.closeModal()"></div>

        <!-- Modal Container -->
        <div class="fixed inset-0 flex items-center justify-center p-4 z-[10000]">
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-5xl z-[10001] max-h-[90vh] overflow-hidden">

                <!-- Header -->
                <div class="relative bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 px-6 py-4 flex justify-between items-center rounded-t-2xl">
                    <div class="flex items-center gap-3">
                        <div class="bg-white/20 backdrop-blur-md border-2 border-white/40 p-2 rounded-xl">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-white">
                                Edit Case: <span id="modal-title-ref" class="font-mono">AA-123-BB</span>
                            </h3>
                            <p class="text-xs text-white/80">
                                <span id="modal-title-name">Customer Name</span> • Order <span id="modal-order-id">#123</span>
                            </p>
                        </div>
                    </div>
                    <button onclick="window.closeModal()" class="text-white/80 hover:text-white hover:bg-white/20 p-2 rounded-lg transition-all">
                        <i data-lucide="x" class="w-5 h-5"></i>
                    </button>
                </div>

                <!-- Workflow Progress -->
                <div class="bg-gradient-to-r from-slate-50 to-purple-50 px-6 py-3 border-b border-slate-200">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-semibold text-slate-700">Workflow Progress</span>
                            <span class="bg-purple-100 text-purple-700 px-2 py-1 rounded-full text-xs font-bold" id="workflow-stage-number">1</span>
                            <span class="text-xs text-slate-500">of 8 stages</span>
                        </div>
                        <div class="text-xs text-slate-600" id="status-description">Initial case import - awaiting processing</div>
                    </div>
                    <div class="w-full bg-slate-200 rounded-full h-2">
                        <div id="workflow-progress-bar" class="bg-gradient-to-r from-purple-500 to-pink-500 h-2 rounded-full transition-all duration-300" style="width: 12.5%"></div>
                    </div>
                </div>

                <!-- Tabs -->
                <div class="flex border-b border-slate-200 bg-slate-50">
                    <button onclick="switchTab('details')" id="tab-details" class="tab-button active flex-1 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition-all border-b-2 border-transparent hover:border-slate-300">
                        <i data-lucide="file-text" class="w-4 h-4 inline mr-2"></i>
                        Case Details
                    </button>
                    <button onclick="switchTab('payments')" id="tab-payments" class="tab-button flex-1 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition-all border-b-2 border-transparent hover:border-slate-300">
                        <i data-lucide="credit-card" class="w-4 h-4 inline mr-2"></i>
                        Payments
                    </button>
                    <button onclick="switchTab('activity')" id="tab-activity" class="tab-button flex-1 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition-all border-b-2 border-transparent hover:border-slate-300">
                        <i data-lucide="activity" class="w-4 h-4 inline mr-2"></i>
                        Activity Log
                    </button>
                    <button onclick="switchTab('notes')" id="tab-notes" class="tab-button flex-1 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition-all border-b-2 border-transparent hover:border-slate-300">
                ...
                <!-- Tab Content -->
                <div class="flex-1 overflow-y-auto max-h-[calc(90vh-200px)] custom-scrollbar">

                    <!-- Details Tab -->
                    <div id="tab-content-details" class="tab-content p-6 space-y-6">
                        <!-- Basic Information -->
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                        <i data-lucide="coins" class="w-4 h-4 text-green-600"></i>
                                        Amount (₾)
                                    </label>
                                    <div class="text-lg font-bold text-green-600" id="modal-amount">0.00</div>
                                </div>
                                <div>
                                    <label class="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                    ...
                                    <div class="text-sm text-slate-600" id="modal-created-date">N/A</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input id="input-phone" type="text" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-purple-400 focus:ring-4 focus:ring-purple-400/20 outline-none">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input id="input-service-date" type="datetime-local" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-orange-400 focus:ring-4 focus:ring-orange-400/20 outline-none">
                                </div>
                                <div>
                                    <label class="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                        <i data-lucide="percent" class="w-4 h-4 text-red-600"></i>
                                        Franchise (₾)
                                    </label>
                                    <input id="input-franchise" type="number" step="0.01" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-red-400 focus:ring-4 focus:ring-red-400/20 outline-none">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select id="input-status" class="w-full appearance-none px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-indigo-400 focus:ring-4 focus:ring-indigo-400/20 outline-none cursor-pointer">
                                        <option value="New">🔵 New Case</option>
                                        <option value="Processing">🟡 Processing</option>
                                        <option value="Called">🟣 Contacted</option>
                        ...
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <button id="btn-call-real" class="flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold text-sm transition-all">
                                    <i data-lucide="phone" class="w-4 h-4"></i>
                                    Call Customer
                                </button>
                                <button id="btn-sms-register" class="flex items-center justify-center gap-2 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-semibold text-sm transition-all">
                                    <i data-lucide="message-square" class="w-4 h-4"></i>
                                    Welcome SMS
                                </button>
                                <button id="btn-sms-arrived" class="flex items-center justify-center gap-2 px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-semibold text-sm transition-all">
                                    <i data-lucide="package" class="w-4 h-4"></i>
                                    Parts Arrived
                                </button>
                            </div>
                            <div class="mt-3">
                                <button id="btn-sms-schedule" class="flex items-center justify-center gap-2 px-4 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-xl font-semibold text-sm transition-all w-full">
                                    <i data-lucide="calendar" class="w-4 h-4"></i>
                    ...
                    <!-- Payments Tab -->
                    <div id="tab-content-payments" class="tab-content hidden p-6 space-y-6">
                        <!-- Payment History -->
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <button onclick="openPaymentModal()" class="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold text-sm transition-all">
                            ...
                            <div id="payments-list" class="space-y-3">
                                ...
                                <div class="text-center py-8">
                                    <div class="bg-slate-100 p-4 rounded-lg">
                                        ...
                                        <span class="text-slate-500 text-sm">No payments recorded yet</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Activity Log Tab -->
                    <div id="tab-content-activity" class="tab-content hidden p-6">
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            <h4 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                                <i data-lucide="activity" class="w-5 h-5 text-blue-600"></i>
                                System Activity Log
                            </h4>
                            <div id="activity-log-container" class="space-y-3 max-h-96 overflow-y-auto custom-scrollbar">
                    ...
                    <!-- Team Notes Tab -->
                    <div id="tab-content-notes" class="tab-content hidden p-6 space-y-6">
                        <!-- Add Note -->
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            <h4 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                            ...
                            <div class="space-y-4">
                                <textarea id="new-note-input" rows="3" placeholder="Add an internal note..." class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm resize-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-400/20 outline-none"></textarea>
                                <button onclick="window.addNote()" class="flex items-center gap-2 px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold text-sm transition-all">
                                    <i data-lucide="plus" class="w-4 h-4"></i>
                                    Add Note
                                </button>
                            </div>
                        ...
                        <div class="bg-white rounded-xl border border-slate-200 p-6">
                            <h4 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                                <i data-lucide="sticky-note" class="w-5 h-5 text-emerald-600"></i>
                                Team Notes
                            </h4>
                ...
                <!-- Special Sections (Customer Review, Reschedule, Confirmation) -->
                <div id="modal-review-section" class="hidden bg-gradient-to-r from-yellow-50 to-orange-50 border-t border-yellow-200 p-4">
                    <div class="flex items-start gap-3">
                        <div class="bg-yellow-100 p-2 rounded-lg">
                            <i data-lucide="star" class="w-5 h-5 text-yellow-600"></i>
                        </div>
                        <div class="flex-1">
                            <h5 class="font-bold text-slate-800 mb-1">Customer Review</h5>
                            <div class="flex items-center gap-1 mb-2" id="modal-review-stars"></div>
                            <div class="text-sm text-slate-700" id="modal-review-comment">No comment provided</div>
                            <div class="text-xs text-slate-500 mt-1">Rating: <span id="modal-review-rating">0</span>/5 stars</div>
                        </div>
                    </div>
                </div>

                <div id="modal-reschedule-section" class="hidden bg-gradient-to-r from-blue-50 to-indigo-50 border-t border-blue-200 p-4">
                    <div class="flex items-start gap-3">
                        <div class="bg-blue-100 p-2 rounded-lg">
                            <i data-lucide="calendar" class="w-5 h-5 text-blue-600"></i>
                        </div>
                        <div class="flex-1">
                            <h5 class="font-bold text-slate-800 mb-1">Reschedule Request</h5>
                            <div class="text-sm text-slate-700 mb-1">Requested: <span id="modal-reschedule-date" class="font-semibold">Not specified</span></div>
                            <div class="text-sm text-slate-700" id="modal-reschedule-comment">No additional comments</div>
                            <div class="flex gap-2 mt-3">
                                <button onclick="acceptReschedule()" class="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-semibold">Accept</button>
                                <button onclick="declineReschedule()" class="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-semibold">Decline</button>
                            </div>
                        </div>
                    </div>
                </div>

                <div id="modal-confirm-section" class="hidden bg-gradient-to-r from-green-50 to-emerald-50 border-t border-green-200 p-4">
                    <div class="flex items-start gap-3">
                        <div class="bg-green-100 p-2 rounded-lg">
                            <i data-lucide="check-circle" class="w-5 h-5 text-green-600"></i>
                        </div>
                        <div class="flex-1">
                            <h5 class="font-bold text-slate-800 mb-1">Manual Confirmation</h5>
                            <div class="text-sm text-slate-700 mb-2">This appointment hasn't been confirmed by the customer yet.</div>
                            <button id="modal-confirm-btn" onclick="manuallyConfirmAppointment()" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-semibold">Confirm Appointment</button>
                        </div>
                    </div>
                </div>
                ...
                <!-- Footer -->
                <div class="px-6 py-4 bg-slate-50 border-t-2 border-slate-200 flex justify-between items-center gap-3 rounded-b-2xl">
                    <button type="button" onclick="window.closeModal()" class="px-6 py-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-200 rounded-xl font-bold text-sm transition-all border-2 border-slate-300">
                        Cancel
                    </button>
                    <button type="button" onclick="window.saveEdit()" class="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2">
    ...
    <!-- Payment Modal -->
    <div id="payment-modal" class="hidden fixed inset-0 z-[10000]" role="dialog" aria-modal="true">
        <!-- Backdrop -->
        <div class="fixed inset-0 bg-gradient-to-br from-slate-900/60 via-green-900/40 to-emerald-900/50 backdrop-blur-lg transition-all duration-300" onclick="closePaymentModal()"></div>

        <!-- Modal Container -->
        <div class="fixed inset-0 flex items-center justify-center p-4 z-[10001]">
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-[10002]">

                <!-- Header -->
                <div class="relative bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 px-6 py-4 flex justify-between items-center rounded-t-2xl">
                    <div class="flex items-center gap-3">
                        <div class="bg-white/20 backdrop-blur-md border-2 border-white/40 p-2 rounded-xl">
                            <i data-lucide="credit-card" class="w-5 h-5 text-white"></i>
                        </div>
                        <div>
                            <h3 class="text-lg font-bold text-white" id="payment-modal-title">Add Payment</h3>
                            <p class="text-xs text-white/80">Record a payment for this case</p>
                        </div>
                    </div>
                    <button onclick="closePaymentModal()" class="text-white/80 hover:text-white hover:bg-white/20 p-2 rounded-lg transition-all">
                        <i data-lucide="x" class="w-5 h-5"></i>
                    </button>
                </div>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input id="payment-amount" type="number" step="0.01" placeholder="0.00" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-green-400 focus:ring-4 focus:ring-green-400/20 outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="payment-date" type="datetime-local" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-blue-400 focus:ring-4 focus:ring-blue-400/20 outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="payment-method" class="w-full appearance-none px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-purple-400 focus:ring-4 focus:ring-purple-400/20 outline-none cursor-pointer">
                    ...
                    <div>
                        ...
                        <textarea id="payment-notes" rows="3" placeholder="Payment reference, receipt number, etc." class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm resize-none focus:border-slate-400 focus:ring-4 focus:ring-slate-400/20 outline-none"></textarea>
                    </div>
                </div>

                <!-- Footer -->
                <div class="px-6 py-4 bg-slate-50 border-t-2 border-slate-200 flex justify-between items-center gap-3 rounded-b-2xl">
                    <button type="button" onclick="closePaymentModal()" class="px-6 py-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-200 rounded-xl font-bold text-sm transition-all border-2 border-slate-300">
                        Cancel
                    </button>
                    <button type="button" id="payment-submit-btn" onclick="savePayment()" class="px-8 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2">
                        <i data-lucide="save" class="w-4 h-4"></i>
                        <span id="payment-submit-text">Add Payment</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        window.openEditModal = (id) => {
            ...
            // Initialize tabs - switch to details tab by default
            switchTab('details');
        ...
        window.switchTab = (tabName) => {
            ...
            tabs.forEach(tab => tab.classList.remove('active', 'border-purple-500', 'text-purple-700'));
            ...
            if (selectedTab) {
                selectedTab.classList.add('active', 'border-purple-500', 'text-purple-700');
            }
        ...
        window.openPaymentModal = (paymentId = null) => {
            ...
            if (paymentId) {
                ...
                if (payment) {
                    ...
                    if (amountEl) amountEl.value = payment.amount;
                    if (dateEl) dateEl.value = payment.payment_date.replace(' ', 'T');
                    if (methodEl) methodEl.value = payment.payment_method;
                    if (notesEl) notesEl.value = payment.notes || '';
                }
            } else {
                // Add mode
        ...
        window.savePayment = async () => {
            ...
            const paymentDate = dateEl ? dateEl.value : '';
            ...
            if (!paymentDate) {
                showToast('Validation Error', 'Payment date is required', 'error');
                if (dateEl) dateEl.focus();
                return;
            }
            if (!paymentMethod) {
                showToast('Validation Error', 'Payment method is required', 'error');
                if (methodEl) methodEl.focus();
                return;
            }
            ...
            try {
                const paymentData = {
                    transfer_id: window.currentEditingId,
                    amount: amount,
                    payment_date: paymentDate.replace('T', ' '),
                ...
                if (window.currentEditingPaymentId) {
                    ...
                    response = await fetchAPI(`update_payment&id=${window.currentEditingPaymentId}`, 'POST', paymentData);
                } else {
                    // Add new payment
                    response = await fetchAPI('add_payment', 'POST', paymentData);
                }
                
                if (response && response.success) {
                    showToast('Success', window.currentEditingPaymentId ? 'Payment updated successfully' : 'Payment added successfully', 'success');
                    closePaymentModal();
                    loadPayments(); // Refresh payments list
                } else {
                    showToast('Error', response?.message || 'Failed to save payment', 'error');
                }
            } catch (error) {
                console.error('Save payment error:', error);
                showToast('Error', 'Failed to save payment', 'error');
            } finally {
                if (submitBtn) {
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = '<i data-lucide="save" class="w-4 h-4"></i> <span id="payment-submit-text">' + (window.currentEditingPaymentId ? 'Update Payment' : 'Add Payment') + '</span>';
                }
            }
        };
        ...
        window.loadPayments = async () => {
            try {
                const response = await fetchAPI(`get_payments&transfer_id=${window.currentEditingId}`, 'GET');
                if (response && response.success) {
                    window.currentPayments = response.payments || [];
                    renderPaymentsList(window.currentPayments);
                } else {
        ...
        window.renderPaymentsList = (payments) => {
            ...
            const paymentsHTML = payments.map(payment => {
                const date = new Date(payment.payment_date.replace(' ', 'T'));
                ...
                const methodIcon = payment.payment_method === 'cash' ? 'coins' : 'building-2';
                const methodColor = payment.payment_method === 'cash' ? 'text-green-600' : 'text-blue-600';
                const methodText = payment.payment_method === 'cash' ? 'Cash' : 'Bank Transfer';
                
                return `
                    <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all">
                        <div class="flex items-start justify-between">
                            <div class="flex items-start gap-3">
                                <div class="bg-slate-100 p-2 rounded-lg">
                                    <i data-lucide="${methodIcon}" class="w-5 h-5 ${methodColor}"></i>
                                </div>
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="text-lg font-bold text-green-600">${payment.amount} ₾</span>
                                        <span class="bg-slate-100 text-slate-600 px-2 py-1 rounded-full text-xs font-medium">${methodText}</span>
                                    </div>
                                    <div class="text-sm text-slate-500 mb-2">${formattedDate}</div>
                                    ${payment.notes ? `<div class="text-sm text-slate-700">${escapeHtml(payment.notes)}</div>` : ''}
                                </div>
                            </div>
                            <div class="flex gap-1">
                                <button onclick="openPaymentModal(${payment.id})" class="text-slate-400 hover:text-blue-600 p-1 rounded transition-all" title="Edit Payment">
                                    <i data-lucide="edit-2" class="w-4 h-4"></i>
                                </button>
                                <button onclick="deletePayment(${payment.id})" class="text-slate-400 hover:text-red-600 p-1 rounded transition-all" title="Delete Payment">
        ...
        window.deletePayment = async (paymentId) => {
            ...
            try {
                const response = await fetchAPI(`delete_payment&id=${paymentId}`, 'POST');
                if (response && response.success) {
                    showToast('Success', 'Payment deleted successfully', 'success');
                    loadPayments(); // Refresh payments list
                } else {
                    showToast('Error', response?.message || 'Failed to delete payment', 'error');
                }
            } catch (error) {
