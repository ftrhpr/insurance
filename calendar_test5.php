...
try {
    ...
    // Build debug output
    $groupingDebug = '<div class="space-y-2">';
    foreach ($casesByDate as $date => $dateCases) {
        $groupingDebug .= '<div class="p-3 bg-yellow-50 rounded border-l-4 border-yellow-400">';
        $groupingDebug .= '<strong class="text-yellow-800">' . htmlspecialchars($date) . '</strong> - ';
        $groupingDebug .= count($dateCases) . ' case(s)<br>';
        foreach ($dateCases as $case) {
            $groupingDebug .= '<span class="text-sm text-yellow-700 ml-4">';
            $groupingDebug .= htmlspecialchars($case['plate'] ?? 'N/A') . ' - ' . htmlspecialchars($case['name'] ?? 'N/A');
            $groupingDebug .= '</span><br>';
        }
        $groupingDebug .= '</div>';
    }
    $groupingDebug .= '</div>';
    ...
    $groupingSuccess = false;
    $groupingDebug = '<p class="text-red-600">Error: ' . htmlspecialchars($e->getMessage()) . '</p>';
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Calendar Test 5 - OTOMOTORS</title>
    ...
    <main class="ml-64 p-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
            ...
            <div class="mb-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
                <p class="text-purple-800">Grouping success: <?php echo $groupingSuccess ? 'YES' : 'NO'; ?></p>
                <p class="text-purple-800">Dates with cases: <?php echo count($casesByDate); ?></p>
                <p class="text-sm text-purple-600">Date range: <?php echo $startDate; ?> to <?php echo $endDate; ?></p>
                <?php if ($groupingSuccess && count($casesByDate) > 0): ?>
                    <div class="mt-4">
                        <p class="text-purple-800 font-semibold">Grouped Cases:</p>
                        <?php echo $groupingDebug; ?>
                    </div>
                <?php endif; ?>
            </div>
