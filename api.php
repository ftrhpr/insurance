...
try {
    ...
    if ($action === 'find_translation_usages' && $method === 'GET') {
        if (!checkPermission('admin')) {
            http_response_code(403);
            jsonResponse(['error' => 'Admin access required']);
        }
        ...
        $text = isset($_GET['text']) ? trim(urldecode($_GET['text'])) : '';
        if (!$key || !$text) jsonResponse(['success' => false, 'message' => 'Missing key or text']);
        ...
        foreach ($rii as $file) {
            ...
            // Exclude paths
            $skip = false;
            foreach ($excludeDirs as $ex) { if (strpos($filePath, DIRECTORY_SEPARATOR.$ex.DIRECTORY_SEPARATOR) !== false) { $skip = true; break; } }
            if ($skip) continue;
            ...
            foreach ($lines as $i => $line) {
                if (strpos($line, $text) !== false) {
                    $matches[] = [
                        ...
                        'line' => $i+1,
                        'context' => trim(mb_substr($line, 0, 400))
                    ];
                }
            }
        }
    ...
    if ($action === 'replace_translation_usages' && $method === 'POST') {
        if (!checkPermission('admin')) {
            http_response_code(403);
            jsonResponse(['error' => 'Admin access required']);
        }
        ...
        $key = trim($data['key'] ?? '');
        $text = trim($data['text'] ?? '');
        $type = trim($data['type'] ?? 'literal'); // 'php' or 'literal'
        ...
        $baseDir = __DIR__;
        $excludeDirs = ['.git', 'vendor', 'fonts', 'node_modules'];
        $allowedExtPhp = ['php','phtml','html','tpl'];
        $allowedExtAll = ['php','html','js','css','txt','tpl','phtml'];

        $summary = ['changed' => [], 'errors' => []];
        ...
        foreach ($rii as $file) {
            ...
            $ext = strtolower(pathinfo($filePath, PATHINFO_EXTENSION));
            if ($type === 'php' && !in_array($ext, $allowedExtPhp)) continue;
            if (!in_array($ext, $allowedExtAll)) continue;
            ...
            // Prepare replacement
            if ($type === 'php') {
                $replacement = "<?php echo __('" . addslashes($key) . "'); ?>";
            } else {
                $replacement = $translatedText;
            }

            $newContent = str_replace($text, $replacement, $content, $count);
            if ($count > 0) {
                // create backup if doesn't exist
                $bak = $filePath . '.bak';
                if (!file_exists($bak)) @copy($filePath, $bak);
                if (@file_put_contents($filePath, $newContent) === false) {
                    $summary['errors'][] = ['file' => substr($filePath, strlen($baseDir)+1), 'message' => 'Failed to write file'];
                } else {
                    $summary['changed'][] = ['file' => substr($filePath, strlen($baseDir)+1), 'replacements' => $count];
                }
            }
        }
