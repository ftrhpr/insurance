...
<body class="bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen">
            ...
            <div class="xl:col-span-1 space-y-6">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100 shadow-sm">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="text-xs font-bold text-blue-900 uppercase tracking-wider">Status</h3>
                    </div>
                    <div class="space-y-3">
                        <select id="input-status" class="w-full appearance-none bg-white border border-blue-200 text-slate-800 py-2 px-3 rounded-lg text-sm font-semibold focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 cursor-pointer">
                        ...
                        <div class="text-xs text-blue-600 font-medium">
                            Amount: <span class="font-bold"><?php echo htmlspecialchars($case['amount']); ?>₾</span>
                        </div>
                    </div>
                </div>

                <!-- Quick Actions Panel -->
                <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-100 shadow-sm">
                    <div class="flex items-center gap-2 mb-3">
                        <div class="bg-emerald-600 p-1.5 rounded-lg shadow-sm">
                            <i data-lucide="zap" class="w-4 h-4 text-white"></i>
                        </div>
                        <h3 class="text-xs font-bold text-emerald-900 uppercase tracking-wider">Quick Actions</h3>
                    </div>
                    <div class="space-y-2">
                        <button id="btn-sms-register" class="w-full flex items-center gap-2 px-3 py-2 bg-white border border-emerald-200 rounded-lg hover:border-emerald-400 hover:bg-emerald-50 transition-all text-left text-sm font-medium text-slate-700">
                            <i data-lucide="message-square" class="w-4 h-4 text-emerald-600"></i>
                            Welcome SMS
                        </button>
                        <button id="btn-sms-arrived" class="w-full flex items-center gap-2 px-3 py-2 bg-white border border-emerald-200 rounded-lg hover:border-emerald-400 hover:bg-emerald-50 transition-all text-left text-sm font-medium text-slate-700">
                            <i data-lucide="package-check" class="w-4 h-4 text-emerald-600"></i>
                            Parts Arrived
                        </button>
                        <button id="btn-sms-schedule" class="w-full flex items-center gap-2 px-3 py-2 bg-white border border-emerald-200 rounded-lg hover:border-emerald-400 hover:bg-emerald-50 transition-all text-left text-sm font-medium text-slate-700">
                            <i data-lucide="calendar-check" class="w-4 h-4 text-emerald-600"></i>
                            Schedule SMS
                        </button>
                        <a id="btn-call-real" href="tel:<?php echo htmlspecialchars($case['phone'] ?? ''); ?>" class="w-full flex items-center gap-2 px-3 py-2 bg-white border border-emerald-200 rounded-lg hover:border-emerald-400 hover:bg-emerald-50 transition-all text-left text-sm font-medium text-slate-700">
                ...
                <div class="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                    <div class="px-3 py-2 bg-gradient-to-r from-slate-700 to-slate-600 flex items-center gap-2">
                        <i data-lucide="history" class="w-3 h-3 text-white"></i>
                        <label class="text-xs font-bold text-white uppercase tracking-wider">Recent Activity</label>
                    </div>
                    <div id="activity-log-container" class="p-3 h-48 overflow-y-auto custom-scrollbar text-xs space-y-2 bg-white/50">
                        <?php
                        if (!empty($case['systemLogs'])) {
                            foreach (array_reverse(array_slice($case['systemLogs'], -5)) as $log) {
                                $date = date('M j, g:i A', strtotime($log['timestamp']));
                                echo "<div class='flex items-start gap-2 p-2 bg-slate-50 rounded border border-slate-200'>";
                                echo "<div class='bg-slate-200 rounded-full p-1 mt-0.5'>";
                                echo "<i data-lucide='activity' class='w-2 h-2 text-slate-600'></i>";
                                echo "</div>";
                                echo "<div class='flex-1'>";
                                echo "<div class='text-xs text-slate-500'>{$date}</div>";
                                echo "<div class='text-xs text-slate-700'>" . htmlspecialchars($log['message']) . "</div>";
                                echo "</div>";
                                echo "</div>";
                            }
                        } else {
                            echo "<div class='text-xs text-slate-500 italic text-center py-4'>No activity recorded</div>";
                        }
                        ?>
                    </div>
                </div>
            ...
            <div class="xl:col-span-3">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 overflow-hidden">
                    <div class="flex border-b border-slate-200">
                        <button onclick="switchTab('details')" id="tab-details" class="tab-button active flex-1 px-6 py-4 text-sm font-semibold text-slate-700 hover:text-blue-600 hover:bg-blue-50 transition-all border-b-2 border-transparent hover:border-blue-200">
                            <i data-lucide="file-text" class="w-4 h-4 inline mr-2"></i>
                            Case Details
                        </button>
                        <button onclick="switchTab('communication')" id="tab-communication" class="tab-button flex-1 px-6 py-4 text-sm font-semibold text-slate-700 hover:text-blue-600 hover:bg-blue-50 transition-all border-b-2 border-transparent hover:border-blue-200">
                            <i data-lucide="message-circle" class="w-4 h-4 inline mr-2"></i>
                            Communication
                        </button>
                        <button onclick="switchTab('feedback')" id="tab-feedback" class="tab-button flex-1 px-6 py-4 text-sm font-semibold text-slate-700 hover:text-blue-600 hover:bg-blue-50 transition-all border-b-2 border-transparent hover:border-blue-200">
                        ...
                        <div id="content-details" class="tab-content">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-1">Customer Name</label>
                                            <input id="input-name" type="text" value="<?php echo htmlspecialchars($case['name']); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 outline-none">
                                        </div>
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-1">Vehicle Plate</label>
                                            <input id="input-plate" type="text" value="<?php echo htmlspecialchars($case['plate']); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 outline-none">
                                        </div>
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-1">Phone Number</label>
                                            <input id="input-phone" type="text" value="<?php echo htmlspecialchars($case['phone'] ?? ''); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 outline-none">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-1">Service Amount</label>
                                            <div class="relative">
                                                <input id="input-amount" type="text" value="<?php echo htmlspecialchars($case['amount']); ?>" class="w-full pl-8 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium focus:border-green-400 focus:ring-2 focus:ring-green-400/20 outline-none">
                                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-green-600 font-bold">₾</span>
                                            </div>
                                        </div>
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-1">Franchise Amount</label>
                                            <input id="input-franchise" type="number" value="<?php echo htmlspecialchars($case['franchise'] ?? 0); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20 outline-none">
                                        </div>
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-1">Service Date & Time</label>
                                            <input id="input-service-date" type="datetime-local" value="<?php echo $case['service_date'] ? date('Y-m-d\TH:i', strtotime($case['service_date'])) : ''; ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium focus:border-purple-400 focus:ring-2 focus:ring-purple-400/20 outline-none">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex gap-3 mt-8 pt-6 border-t border-slate-200">
                                <button onclick="saveChanges()" class="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-3 px-6 rounded-xl font-bold text-sm shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2">
                                    <i data-lucide="save" class="w-4 h-4"></i>
                                    Save Changes
                                </button>
                                <button onclick="printCase()" class="bg-slate-600 hover:bg-slate-700 text-white py-3 px-6 rounded-xl font-bold text-sm shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2">
                                    <i data-lucide="printer" class="w-4 h-4"></i>
                                    Print
                                </button>
                                <button onclick="deleteCase()" class="bg-red-600 hover:bg-red-700 text-white py-3 px-6 rounded-xl font-bold text-sm shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2">
                        ...
                        <div id="content-communication" class="tab-content hidden">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-2">Select Template</label>
                                            <select id="sms-template-selector" class="w-full bg-white border border-slate-200 rounded-lg p-3 text-sm font-medium focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 outline-none">
                                        ...
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-2">Message Preview</label>
                                            <div id="sms-preview" class="bg-slate-50 border border-slate-200 rounded-lg p-4 min-h-[100px] text-sm text-slate-700 whitespace-pre-wrap">
                                                <span class="text-slate-400 italic">Select a template to see preview...</span>
                                            </div>
                                        </div>
                                        <button id="btn-send-custom-sms" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed" disabled>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 max-h-64 overflow-y-auto">
                                        <div class="space-y-3 text-sm">
                                            <?php
                                            if (!empty($case['systemLogs'])) {
                                                $commLogs = array_filter($case['systemLogs'], function($log) {
                                                    return strpos(strtolower($log['message']), 'sms') !== false || strpos(strtolower($log['message']), 'call') !== false;
                                                });
                                                if (!empty($commLogs)) {
                                                    foreach (array_reverse($commLogs) as $log) {
                                                        $date = date('M j, g:i A', strtotime($log['timestamp']));
                                                        echo "<div class='flex items-start gap-3 p-2 bg-white rounded border border-slate-100'>";
                                                        echo "<div class='bg-teal-100 p-1 rounded'>";
                                                        echo "<i data-lucide='message-circle' class='w-3 h-3 text-teal-600'></i>";
                                                        echo "</div>";
                                                        echo "<div class='flex-1'>";
                                                        echo "<div class='text-xs text-slate-500'>{$date}</div>";
                                                        echo "<div class='text-slate-700'>" . htmlspecialchars($log['message']) . "</div>";
                                                        echo "</div>";
                                                        echo "</div>";
                                                    }
                                                } else {
                                                    echo "<div class='text-slate-500 italic text-center py-4'>No communication logs yet</div>";
                                                }
                                            } else {
                                                echo "<div class='text-slate-500 italic text-center py-4'>No communication logs yet</div>";
                                            }
                        ...
                        <div id="content-feedback" class="tab-content hidden">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div id="review-display" class="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-lg border border-amber-200 p-4">
                                        ...
                                        <div class="flex items-center gap-4 mb-3">
                                            ...
                                            <span class="text-2xl font-black text-amber-600"><?php echo $case['reviewStars']; ?>/5</span>
                                        </div>
                                    ...
                                    <button id="btn-edit-review" class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded-lg transition-all shadow-lg active:scale-95">
                                        <i data-lucide="edit" class="w-4 h-4 inline mr-2"></i>
                                        Edit Review
                                    </button>

                                    <!-- Review Edit Form (Hidden by default) -->
                                    <div id="review-edit" class="bg-white border border-slate-200 rounded-lg p-4 space-y-3 hidden">
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-2">Rating</label>
                                            <select id="input-review-stars" class="w-full bg-slate-50 border border-slate-200 rounded p-2 text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20 outline-none">
                                        ...
                                        <div>
                                            <label class="block text-xs text-slate-500 font-bold uppercase mb-2">Comment</label>
                                            <textarea id="input-review-comment" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded p-2 text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20 outline-none resize-none"><?php echo htmlspecialchars($case['reviewComment'] ?? ''); ?></textarea>
                                        </div>
                                        <div class="flex gap-2">
                                            <button id="btn-save-review" class="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-3 rounded transition-all active:scale-95">
                                                Save
                                            </button>
                                            <button id="btn-cancel-review" class="px-3 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded transition-all">
                                                Cancel
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="bg-white border border-slate-200 rounded-lg overflow-hidden">
                                        <div class="p-4">
                                            <div id="notes-container" class="space-y-3 mb-4 max-h-64 overflow-y-auto custom-scrollbar">
                                                ...
                                                if (!empty($case['internalNotes'])) {
                                                    foreach ($case['internalNotes'] as $note) {
                                                        ...
                                                        echo "<div class='bg-slate-50 p-3 rounded border border-slate-100'>";
                                                        echo "<p class='text-sm text-slate-700'>" . htmlspecialchars($note['text']) . "</p>";
                                                        echo "<div class='flex justify-end mt-2'>";
                                                        echo "<span class='text-xs text-slate-400 bg-white px-2 py-0.5 rounded border'>" . htmlspecialchars($note['authorName'] ?? 'Manager') . " - {$date}</span>";
                                                        echo "</div>";
                                                        echo "</div>";
                                                    }
                                                } else {
                                            ...
                                            <div class="flex gap-2">
                                                <input id="new-note-input" type="text" placeholder="Add a note..." class="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded text-sm focus:border-slate-400 focus:ring-2 focus:ring-slate-400/20 outline-none">
                                                <button onclick="addNote()" class="bg-slate-600 hover:bg-slate-700 text-white px-4 py-2 rounded font-bold text-sm transition-all active:scale-95">
                            ...
                            <div class="mt-6 bg-gradient-to-br from-purple-50 to-fuchsia-50 rounded-xl border-2 border-purple-200 overflow-hidden shadow-lg">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize default tab (Case Details)
            switchTab('details');
