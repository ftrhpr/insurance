...
// Simple test - just try to fetch cases count
$caseCount = 0;
try {
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM transfers WHERE due_date IS NOT NULL");
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $caseCount = $result['count'] ?? 0;
} catch (PDOException $e) {
    $caseCount = -1; // Error indicator
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Calendar Test 2 - OTOMOTORS</title>
    ...
    <main class="ml-64 p-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
            ...
            <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p class="text-blue-800">Total cases with due dates: <?php echo $caseCount; ?></p>
                <p class="text-sm text-blue-600">If you see a number above, database queries are working.</p>
            </div>
